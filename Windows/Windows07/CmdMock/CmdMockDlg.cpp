...
void CCmdMockDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (PeekNamedPipe(m_hParentRead, NULL, 0, NULL, &dwBytesAvail, NULL))
	{
		...
		{
			CString lastText;
			GetDlgItemText(EDT_RESULT, lastText);
			DWORD dwByteReaded = 0;
			CString strBuf;
			auto bRet = ReadFile(m_hParentRead,
				strBuf.GetBufferSetLength(MAXBYTE),
				MAXBYTE,
				&dwByteReaded,
				NULL);
			strBuf.ReleaseBuffer(dwByteReaded);
			SetDlgItemText(EDT_RESULT, lastText + strBuf);
			m_resultEdit.LineScroll(m_resultEdit.GetLineCount());
		}
	}
