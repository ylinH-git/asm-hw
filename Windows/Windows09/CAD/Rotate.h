...
inline void rotate(CPoint& beginPos,CPoint& endPos,double dblR)
{
  ...
  pos.x = (beginPos.x - x) * cos(dblR) - (beginPos.y - y) * sin(dblR) + x;
  pos.y = (beginPos.y - y) * cos(dblR) + (beginPos.x - x) * sin(dblR) + y;
  ...
  pos.x = (endPos.x - x) * cos(dblR) - (endPos.y - y) * sin(dblR) + x;
  pos.y = (endPos.y - y) * cos(dblR) + (endPos.x - x) * sin(dblR) + y;
  endPos = pos;
}
