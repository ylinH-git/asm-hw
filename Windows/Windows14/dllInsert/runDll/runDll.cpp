...
{
    // 找到窗口
    HWND hwndWinmine = FindWindow(NULL, "摿孭");
    // 获取进程ID
    ...
    // 写入dll地址
    char szDllPath[] = { R"(E:\科锐逆向培训\作业\Windows\Windows14\dllInsert\Debug\myDll.dll)" };
    ...
    if (pAddr)
    {
        VirtualFreeEx(proHdWinmine, pAddr, 0, MEM_RELEASE);
    }
