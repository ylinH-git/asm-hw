...
void CAttrDlg::InitHookWin()
{
	m_hookWin.Create(DLG_HOOK, &m_tab);

	DWORD processID;
	DWORD threadID;
	threadID = ::GetWindowThreadProcessId(m_currHwnd, &processID);
	m_hookWin.m_processId = processID;
	m_hookWin.m_threadId = threadID;
	m_hookWin.m_curHwnd = m_currHwnd;
}
