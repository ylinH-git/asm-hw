...
)
{
	...
	while (true)
	{
		...
		else
		{
			...
			int nRet = recvfrom(dlg->m_socket, (char*)&pkg, sizeof(pkg), 0, (sockaddr*)&siRecv, &nSiSize);
			...
			{
				//显示到界面
				CString loginUser = pkg.m_szName;
				...
				int nIdx = dlg->m_contactList.AddString(pkg.m_szName);
				dlg->m_contactList.SetItemData(nIdx, (DWORD_PTR)new sockaddr_in(pkg.m_siFrom));

				break;
			}
			case S2C_LOGOUT:
			{
				CString logoutUserName = pkg.m_szName;
...
void CclientDlg::OnBnClickedLogin()
{
	...
	Package pkg(C2S_LOGIN, m_loginName.GetBuffer(0));
	m_siServer.sin_family = AF_INET;
	m_siServer.sin_port = htons(8888);
	m_siServer.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");
	BOOL nRet = sendto(m_socket, (char*)&pkg, sizeof(pkg), 0, (sockaddr*)&m_siServer, sizeof(m_siServer));
...
void CclientDlg::OnBnClickedLogout()
{
	...
	//向服务发送下线包
	Package pkg(C2S_LOGOUT, m_loginName.GetBuffer(0));
	sendto(m_socket, (char*)&pkg, sizeof(pkg), 0, (sockaddr*)&m_siServer, sizeof(m_siServer));
}
...
void CclientDlg::OnBnClickedSend()
{
	...
	//发送公聊信息
	Package pkg(C2S_PUBMSG, m_loginName.GetBuffer(), m_speakInfo.GetBuffer());
	sendto(m_socket, (char*)&pkg, sizeof(pkg), 0, (sockaddr*)&m_siServer, sizeof(m_siServer));
	m_speakInfo = "";
	UpdateData(FALSE);
}
...
void CclientDlg::OnBnClickedSendPrivate()
{
	...
	Package pkg(C2S_PRIMSG, m_loginName.GetBuffer(), {}, *pSI, m_speakInfo.GetBuffer());
	sendto(m_socket, (char*)&pkg, sizeof(pkg), 0, (sockaddr*)&m_siServer, sizeof(m_siServer));
	m_speakInfo = "";
	UpdateData(FALSE);
}
...
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	Package pkg(C2S_HEART, m_loginName .GetBuffer());
	sendto(m_socket, (char*)&pkg, sizeof(pkg), 0, (sockaddr*)&m_siServer, sizeof(m_siServer));
